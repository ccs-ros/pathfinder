...
public:
  ...
  //void getIMUCallback(const navigation::FilterInput::ConstPtr &msg) {
    ...
    //convert to rad/s and adjust coordinate system
    this->q = msg->Gx*(PI/180);
    this->p = msg->Gy*(PI/180);
    this->r = msg->Gz*(PI/180);
    //convert to inches/s^2 and adjust coordinate system
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
    	...
    	cout << "p = " << inData.p << "q = " << inData.q << "r = " << inData.r << endl;
    	cout << "ax = " << inData.ax << "ay = " << inData.ay << "az = " << inData.az << endl;
