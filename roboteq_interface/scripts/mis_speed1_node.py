...
from roboteq_interface.msg import speed1_data
import serial
import struct
import time
...
def openPort():
	...
	ser.flushInput()
	ser.flushOutput()
	#rospy.loginfo("Opened Controller 1")
...
def mis_speed1_node():
	...
	global port_open
	global callback_time
	pub = rospy.Publisher('mis/speed1_data', speed1_data)
	sub = rospy.Subscriber('mis/motor_commands',motor_commands,callback)
	rospy.init_node('mis_speed1_node')
	msg_out = speed1_data()
	motor_1_speed = 0
	motor_2_speed = 0
	...
	while not port_open:
		try:
			...
			#rospy.loginfo("Controller 1 not open")
			pass
