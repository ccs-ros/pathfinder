...
int main(int argc, char **argv)
{
    ...
    enum modes_t {Pause, Init, Travel_to_search, Search, Approach, Re_orient, Collect, Confirm_collect, Return_to_path, Return_to_base, Deposit, Confirm_deposit, Lost, Finish} mode;
    mode = Init;
    static modes_t previous_mode = mode;
    while(ros::ok())
    {
    	cout << "mode " << mode << endl;
    	switch(mode)
    	{
    		...
    		case Init:
    			...
    			actuators.gimbal.hold(&robot_status);
    			previous_mode = mode;
    			ros::Duration(5).sleep(); // wait for 5 seconds for initialization
    			mode = Search;
